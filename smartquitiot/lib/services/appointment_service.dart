...
class AppointmentService {
  ...
  Future<Map<String, dynamic>> bookAppointment(
      Map<String, dynamic> reqBody, String accessToken) async {
  ...
  Future<List<dynamic>> getMyAppointments(String accessToken) async {
    ...
    http.Response resp;
    try {
      resp = await http.get(Uri.parse(url), headers: headers).timeout(const Duration(seconds: 20));
    } catch (e) {
      throw Exception('Network error: $e');
    }
    ...
    debugPrint('[AppointmentService] GET $url -> status=${resp.statusCode} bodyType=${body?.runtimeType}');
  ...
  Future<Map<String, dynamic>> requestJoinToken(int appointmentId, String accessToken) async {
    ...
    try {
      ...
      resp = await http.post(Uri.parse(url), headers: headers, body: jsonEncode({})).timeout(const Duration(seconds: 15));
    } catch (e) {
      throw Exception('Network error: $e');
    }
